...
2. **Testnet funding path — genuinely blocking, needs a human.** `algokit` CLI is not installed in this environment, and `algokit dispenser fund` requires an interactive OAuth login (device flow) only the human account owner can complete. The public faucet (`bank.testnet.algorand.network`) is captcha-gated and not scriptable. Two addresses were generated (`agent/src/keygen.ts`) and are already in `.env` (gitignored, not committed):
   - **Resource server** `AVM_ADDRESS = QWNOD7MX4JLAN3P4U5ASICNBZV3QRGU4C3R6YMTFUZCFCUFW44UOPKLYCM` — needs a small amount of testnet ALGO (≥ 0.1 ALGO) so it can opt in to the USDC ASA (`10458941`) and receive payments. Opt-in is a one-time step (not yet automated — will land with Phase 2's wallet-provisioning code, or can be done manually via `algokit`/Lora/Pera against testnet).
   - **Paying agent** `AVM_PRIVATE_KEY` → address `S32MEQLEZUZUCXMAR5CGOWKZWHTJNZJTK2UAIH3JTT3JEIZOXQYOQ57VMM` — needs testnet ALGO (for USDC opt-in minimum balance; the facilitator's `feePayer` may cover the payment tx's own fee, unconfirmed until tested) **and** testnet USDC (ASA `10458941`) to actually pay the $0.01 route.
   - Once both are funded and both addresses have opted in to ASA `10458941`, run `npm run start --workspace agent` from repo root — `agent/src/test-payment.ts` runs the full Phase 1 hard-gate check (402 → pay → 200) and prints the settlement tx id + Lora explorer link.
